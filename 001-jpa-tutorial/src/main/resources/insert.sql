...
--select s.sale_date,p.name,sd.qty,sum(sd.qty*sd.total) from sale s 
--join sale_details sd on sd.sales_id=s.id 
--join product p on p.id=sd.product_id 
--where s.sale_date between '2022-01-12' and '2023-05-01' 
--group by s.sale_date,p.name,sd.qty;
--
--
--CountProductByTownship
--select a.township,p.name,p.ws_price,sd.qty,sum(p.ws_price*sd.qty) from address a 
--join customer c on c.address_id = a.id 
--join sale s on s.customer_id=c.id 
--join sale_details sd on sd.sales_id=s.id 
--join product p on p.id=sd.product_id 
--group by a.township,p.name,p.ws_price,sd.qty;